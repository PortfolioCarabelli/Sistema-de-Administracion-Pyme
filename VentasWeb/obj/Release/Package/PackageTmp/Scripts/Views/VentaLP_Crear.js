...
$(document).ready(function () {
    ...
    tablaproducto = $('#tbProducto').DataTable({
        "ajax": {
            "url": $.MisUrls.url._ObtenerProductoStockPorClienteLP + "?IdTienda=" + $("#txtIdTienda").val() + "&IdCliente=" + $("#txtIdCliente").val(),
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            {
                "data": "oListaPrecio", render: function (data) {
                    return data.PrecioCliente
                }
            },
...
function productoSelect(json) {
    ...
    $("#txtproductoprecio").val(json.oListaPrecio.PrecioCliente);
    $("#txtproductocantidad").val("0");
...
$("#txtproductocodigo").on('keypress', function (e) {
    ...
    if (e.which == 13) {
        ...
        var request = { IdTienda: parseInt($("#txtIdCliente").val()) }

        //OBTENER PROVEEDORES
        jQuery.ajax({
            url: $.MisUrls.url._ObtenerProductoStockPorClienteLP + "?IdTienda=" + parseInt($("#txtIdTienda").val()) + "&IdCliente=" + $("#txtIdCliente").val(),
            type: "GET",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            success: function (data) {
                ...
                if (data.data != null) {
                    $.each(data.data, function (i, item) {
                        if (item.oProducto.Codigo == $("#txtproductocodigo").val()) {
                            ...
                            $("#txtproductoprecio").val(item.oListaPrecio.PrecioCliente);
                            encontrado = true;
                            return false;
                        }
...
$('#btnTerminarGuardarCuentaCorriente').on('click', function () {
    ...
    DETALLE = "<DETALLE>" + VENTA + DETALLE_CLIENTE + DETALLE_VENTA + CUENTACORRIENTE + "</DETALLE>"


    var request = { xml: DETALLE };

    jQuery.ajax({
        url: $.MisUrls.url._RegistrarCuentaCorriente,
        type: "POST",
        data: JSON.stringify(request),
        dataType: "json",
        contentType: "application/json; charset=utf-8",
