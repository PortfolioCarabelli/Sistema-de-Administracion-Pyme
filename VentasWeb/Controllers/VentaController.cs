...
namespace VentasWeb.Controllers
{
    public class VentaController : Controller
    {
        ...
        public ActionResult CrearCuentaCorriente()
        {
            SesionUsuario = (Usuario)Session["Usuario"];
            return View();
        }
        public ActionResult CrearVentaLP()
        {
            SesionUsuario = (Usuario)Session["Usuario"];
            return View();
        }
        ...
        public JsonResult ObtenerUsuario1()
        {
            Usuario rptUsuario = CD_Usuario.Instancia.ObtenerDetalleUsuario(SesionUsuario.IdUsuario);
            return Json(rptUsuario, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult ObtenerProductoCliente(int IdTienda, int idCliente)
        {

            List<ProductoCliente> oListaProductoCliente = CD_ListaPrecios.Instancia.ObtenerProductoClienteLP(idCliente);
            oListaProductoCliente = oListaProductoCliente.Where(x => x.oTienda.IdTienda == IdTienda && x.Stock > 0).ToList();


            return Json(new { data = oListaProductoCliente }, JsonRequestBehavior.AllowGet);
        }
        ...
        [HttpPost]
        public JsonResult GuardarCuentaCorriente(string xml)
        {
            xml = xml.Replace("!idusuario¡", SesionUsuario.IdUsuario.ToString());
            int Respuesta = 0;
            Respuesta = CD_Venta.Instancia.RegistrarCuentaCorriente(xml);
            if (Respuesta != 0)
                return Json(new { estado = true, valor = Respuesta.ToString() }, JsonRequestBehavior.AllowGet);
            else
                return Json(new { estado = false, valor = "" }, JsonRequestBehavior.AllowGet);
        }
        public JsonResult GuardarCuentaCorriente1(string xml)
        {
            xml = xml.Replace("!idusuario¡", SesionUsuario.IdUsuario.ToString());
            int Respuesta = 0;
            Respuesta = CD_Venta.Instancia.RegistrarCuentaCorriente(xml);
